	...
	$("input[type=submit]").click(function(){
		...
		var fileName3 = $("input#fileName3").val();
			if(fileName1.length==0){
				$("#tixing1").html("对不起 文件名不可以为空");
			}else{
				$("#tixing1").html("");
			}
			if(fileName2.length==0){
				$("#tixing2").html("对不起 文件名不可以为空");
			}else{
				$("#tixing2").html("");
			}
			if(fileName3.length==0){
				$("#tixing3").html("对不起 文件名不可以为空");
			}else{
				$("#tixing3").html("");
			}
...
<body>
	<form action="encorchast.sys?method=edit" method="post">
	<input type="hidden" name="encOrChastId" value="${encorchast.encOrChastId }"/>
		<table class="toptable_style">
			<tr>
				<td width="85%"><img src="/personnel/comm/images/comm/tb.gif" />
					<span class="title_style">修改奖惩制度信息</span></td>
		...
		<table class="centtable_style" cellspacing="1px">
			...
			<tr>
				...
				<td class="centtr"><input size="40" type="text" value="${encorchast.personnelEncOrChastPoint }" name="personnelEncOrChastPoint" id = "fileName1"/><span style="color:red;" id="tixing1"></span></td>
			</tr>
			<tr>
				<td class="toptr">奖惩金额</td>
				<td class="centtr"><input size="40" type="text" value="${encorchast.money }" name="money" id = "fileName2"/><span style="color:red;" id="tixing2"></span></td>
			</tr>
			<tr>
				<td class="toptr">奖惩级别</td>
				<td class="centtr"><input size="40" type="text" value="${encorchast.personnelEncOrChastSort }" name="personnelEncOrChastSort" id= "fileName3"/><span style="color:red;" id="tixing3"></span></td>
			</tr>
		</table>
	</form>
