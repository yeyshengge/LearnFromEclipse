...
function submitForm2(){
	...
	if(departmentName!=""&&jobName==""){
		window.location.href="personAppraise.per?method=findAll&state=1&currentPage=1&departmentName="+departmentName;
	}else if(departmentName==""&&jobName!=""){
		window.location.href="personAppraise.per?method=findAll&state=2&currentPage=1&jobName="+jobName;
	}else{
		alert("请重新填写查询条件！");
	}
...
function getPage() {
	...
	for ( var k = 0; k < page.length; k++) {
		var b1 = page.charAt(k);//返回指定索引处的 char 值
		var b = b1.charCodeAt();//返回指定位置的字符的 Unicode 编码
		if (b < 48 || b > 58) {
			i=i+1;
		}
	}
	if(i>0&&pageLen>0){
		...
		window.location.href="personAppraise.per?method=findAll&state=1&currentPage="+page+"&departmentName="+departmentName;
	}
}
