...
<body>
<c:if test="${flagTrain==null}">
		<jsp:forward page="/user_info.inf?method=queryTrain"></jsp:forward>
</c:if>
	<form action="" style="width: 100%">
		<table class="toptable_style">
			<tr>
				<td width="85%"><img src="/personnel/comm/images/comm/tb.gif" />
					<span class="title_style">员工培训详细信息</span></td>
				<td><span class="title_style"> <input type="checkbox"
						name="selectall" id="selectall" onclick="selectAll()"/> 全选/全不选
				</span> &nbsp;
				<span class="title_style"><a href="#" class="aset"  onclick="popwinTrain()"> 打印</a> </span> </td>
			</tr>
		</table>
		...
		<c:forEach var="var" items="${showPersonTrain}">
			<tr class="centtr">
				<td><input type="checkbox" name="checkbox" id="checkbox" /></td>
				<td>${var.personnelinfo.personnelId}</td>
				<td>${var.personnelTrainDate}</td>
				<td>${var.personnelTrainConten}</td>
				<td>${var.personnelTrainRemark}</td>
			</tr>
			</c:forEach>
		</table>
	<jsp:include page="/WEB-INF/jsps/information/pageTrain.jsp"></jsp:include>
	</form>
</body>
</html>