...
<body>
<c:if test="${flagEn==null}">
		<jsp:forward page="/user_info.inf?method=queryEn"></jsp:forward>
</c:if>
	<form action="" style="width: 100%">
		<table class="toptable_style">
			<tr>
				<td width="85%"><img src="/personnel/comm/images/comm/tb.gif" />
					<span class="title_style">奖惩资料详细信息</span></td>
		...
		<c:forEach var="var" items="${showPersonEn}">
			<tr class="centtr">
				<td><input type="checkbox" name="checkbox" id="checkbox" /></td>
				<td>${var.personnelinfo.personnelId}</td>
				<td>${var.peocDate}</td>
				<td>${var.peocReason}</td>
				<td>${var.personnelTrainRemark}</td>
			</tr>
			</c:forEach>
		</table>
	<jsp:include page="/WEB-INF/jsps/information/pageEn.jsp"></jsp:include>
	</form>
</body>
</html>