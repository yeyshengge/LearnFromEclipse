...
<script type="text/javascript"
	src="/personnel/comm/js/person/personAppraise/personAppraise.js"></script>
<script language="javascript" type="text/javascript"
	src="comm/calenderController/My97DatePicker/WdatePicker.js"></script>
<link rel="text/javascript">

<link type="text/css"
	href="comm/css/person/personAppraise/personAppraise.css"
	rel="stylesheet" />
<title>员工好评管理</title>
</head>
<body>
	...
	<div>
		<div id="pAppraiseTitle">
			<div id=titleText>人事管理&nbsp;&nbsp;&nbsp;员工好评管理：</div>
			<div class="titleButton">
				<img class="titleButtonImage"
					src="/personnel/comm/images/comm/add.gif"> <a
					class="titleButtonText" onClick="locking()">新增</a> <a href=""
					class="titleButtonText1">导入Excel</a>
				<!-- <input type="file" /> -->
		...
		<div id="query" align="center">
			...
			<a href="personAppraise.per?method=findAll&state=0&currentPage=1">查询所有</a>
			<form id="findByCondition">
				 按部门查询：<input type="text" name="departmentName">
				 按职务查询：<input type="text" name="jobName"> <input type="button" value="查询" onclick="submitForm2();">
			</form>
			<table class="tableQuery">
				...
				<c:forEach var="newls" items="${ls }">
					<tr class="tr2">
						...
						<td>${newls.personnelAppraiseContent }</td>
						<td>${fn:substring(newls.personnelAppraiseDate,0,19) }</td>
						<td>${newls.personnelAppraiseResult }</td>
						<td>${newls.personnelAppraiseRemark }</td>
						<td><a
							href="personAppraise.per?method=findSingle&personnelAppraiseId=${newls.personnelAppraiseId }&personnelName=${newls.personnelinfo.personnelName }&personnelAppraiseContent=${newls.personnelAppraiseContent }&personnelAppraiseDate=${newls.personnelAppraiseDate }&personnelAppraiseResult=${newls.personnelAppraiseResult }&personnelAppraiseRemark=${newls.personnelAppraiseRemark }&state=1">删除</a>&nbsp;
							<a
							href="personAppraise.per?method=findSingle&personnelAppraiseId=${newls.personnelAppraiseId }&personnelName=${newls.personnelinfo.personnelName }&state=0">修改</a>
						</td>
				...
				<table width="312" border="0" style="margin-left: 153px;"
					...
					<tr>
						<td width="49"><div align="center">
								<a href="personAppraise.per?method=findAll&state=0&currentPage=1">
						...
						<c:if test="${currentPage ne 1 }">
							<td width="49">
								<div align="center">
									<a
										href="personAppraise.per?method=findAll&state=0&currentPage=${currentPage-1 }">
						...
						<td width="49"><div align="center">
								<a
									href="personAppraise.per?method=findAll&state=0&currentPage=${allPage }">
		...
		<div id="Layer2" align="center">
			<table align="center">
				...
				<tr>
					<td>
						<div>
							<form id="formid" action="personAppraise.per?method=save"
								method="post">
								<div id="personnelNo">
								...
								<div id="personnelAppraiseContent">
									考核内容：<input id="input2" name="input2" type="text"
										onblur="getContent();"><span id="span2"
										style="color: red; font-size: smaller;"></span>
								</div>
								<div id="personnelAppraiseDate">
									考核时间：<input id="input3" name="input3" type="text"
										onblur="getTime();" onclick="WdatePicker();"><span
										id="span3" style="color: red; font-size: smaller;"></span>
								</div>
								<div id="personnelAppraiseResult">
									考核结果：<select name="input4">
										<option>优秀</option>
										<option>良好</option>
										<option>中等</option>
										<option>合格</option>
								...
								<div id="personnelAppraiseRemark">
